...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    for key, metric_name in [
        ("valid_action_stats", "env/number_of_valid_action"),
        ("valid_search_stats", "env/number_of_valid_search"),
        ("valid_vision_search_stats", "env/number_of_valid_vision_search"),
    ]:
        if key in batch.non_tensor_batch:
            values = np.asarray(batch.non_tensor_batch[key], dtype=np.float32)
            metrics[f"{metric_name}/mean"] = values.mean()
